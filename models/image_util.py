...
def jaccard_overlap(sample_bbox, object_bbox):
    if (sample_bbox.xmin >= object_bbox.xmax
...
def satisfy_sample_constraint(sampler, sample_bbox, bbox_labels):
    ...
    for i in range(len(bbox_labels)):
        ...
        if (sampler.min_jaccard_overlap != 0
                and overlap < sampler.min_jaccard_overlap):
            continue
        if (sampler.max_jaccard_overlap != 0
...
def meet_emit_constraint(src_bbox, sample_bbox):
    ...
    if (center_x >= sample_bbox.xmin and center_x <= sample_bbox.xmax
            and center_y >= sample_bbox.ymin and center_y <= sample_bbox.ymax):
        return True
    return False
